class LocationsController < ApplicationController
  def index
    ...
    @locations = @q.result(:distinct => true).includes(:catalogs, :wines).page(params[:page]).per(10)
  ...
  def create
    ...
    @location.establishment_type = params[:establishment_type]
    @location.name = params[:name]
    @location.address = params[:address]
    @location.city = params[:city]
    @location.time_open = params[:time_open]
    @location.time_close = params[:time_close]
